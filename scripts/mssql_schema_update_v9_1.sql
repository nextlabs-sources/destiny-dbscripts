...
ALTER TABLE TOOL_PARAMETER ALTER COLUMN PATTERN VARCHAR(1024);
UPDATE TOOL_PARAMETER SET PATTERN = '[^~/*$&?\\\\]*' WHERE COMMAND_ID=(SELECT COMMAND_ID FROM TOOL_COMMAND_VIEW WHERE TOOL_COMMAND_NAME='enrollmgr' AND COMMAND_NAME='-enroll') AND COMMAND_NAME = '-n';
UPDATE TOOL_PARAMETER SET PATTERN = '^(?:(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[^A-Za-z0-9]))(?!.*(.)\1{2,})[A-Za-z0-9!~`<>,&semi:_=?*+#.\\''\x22&%()\|\[\]\{\}\-\$\^\@\/]{10,128}$' WHERE COMMAND_ID=(SELECT COMMAND_ID FROM TOOL_COMMAND_VIEW WHERE TOOL_COMMAND_NAME='enrollmgr' AND COMMAND_NAME='-enroll') AND COMMAND_NAME = '-w';
UPDATE TOOL_PARAMETER SET PATTERN = '^(?:(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[^A-Za-z0-9]))(?!.*(.)\1{2,})[A-Za-z0-9!~`<>,&semi:_=?*+#.\\''\x22&%()\|\[\]\{\}\-\$\^\@\/]{10,128}$' WHERE COMMAND_ID=(SELECT COMMAND_ID FROM TOOL_COMMAND_VIEW WHERE TOOL_COMMAND_NAME='propertymgr' AND COMMAND_NAME='-add') AND COMMAND_NAME = '-w';
