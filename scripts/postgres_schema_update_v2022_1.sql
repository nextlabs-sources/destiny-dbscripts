...
INSERT INTO SYS_CONFIG (ID, APPLICATION, CONFIG_KEY, VALUE, DEFAULT_VALUE, VALUE_FORMAT, MAIN_GROUP, SUB_GROUP, MAIN_GROUP_ORDER, SUB_GROUP_ORDER, CONFIG_ORDER, HIDDEN, READ_ONLY, ADVANCED, UI, ENCRYPTED, RESTART_REQUIRED, DATA_TYPE, FIELD_TYPE, OPTIONS, REQUIRED, PATTERN, DESCRIPTION, MODIFIED_ON, MODIFIED_BY ) VALUES (NEXTVAL('HIBERNATE_SEQUENCE'), 'application', 'cc-oidc-config.services[0].serviceId', '${server.name}(:\d*)?\/.*', '${server.name}(:\d*)?\/.*', '', 'security', 'cas', '40', '5', '0', '1', '1', '1', '0', '0', '0', 'text', 'text', '', '1', '', 'OIDC service id', CURRENT_TIMESTAMP, '-1' )
...
INSERT INTO SYS_CONFIG (ID, APPLICATION, CONFIG_KEY, VALUE, DEFAULT_VALUE, VALUE_FORMAT, MAIN_GROUP, SUB_GROUP, MAIN_GROUP_ORDER, SUB_GROUP_ORDER, CONFIG_ORDER, HIDDEN, READ_ONLY, ADVANCED, UI, ENCRYPTED, RESTART_REQUIRED, DATA_TYPE, FIELD_TYPE, OPTIONS, REQUIRED, PATTERN, DESCRIPTION, MODIFIED_ON, MODIFIED_BY ) VALUES (NEXTVAL('HIBERNATE_SEQUENCE'), 'service-manager', 'spring.security.oauth2.resourceserver.jwt.issuer-uri', '${server.name}/cas/oidc', '${server.name}/cas/oidc', '', 'security', 'service-manager', '40', '120', '0', '1', '1', '1', '0', '0', '0', 'text', 'text', '', '1', '', 'JWT Issuer URI', CURRENT_TIMESTAMP, '-1' )
...
INSERT INTO SYS_CONFIG (ID, APPLICATION, CONFIG_KEY, VALUE, DEFAULT_VALUE, VALUE_FORMAT, MAIN_GROUP, SUB_GROUP, MAIN_GROUP_ORDER, SUB_GROUP_ORDER, CONFIG_ORDER, HIDDEN, READ_ONLY, ADVANCED, UI, ENCRYPTED, RESTART_REQUIRED, DATA_TYPE, FIELD_TYPE, OPTIONS, REQUIRED, PATTERN, DESCRIPTION, MODIFIED_ON, MODIFIED_BY ) VALUES (NEXTVAL('HIBERNATE_SEQUENCE'), 'service-manager', 'spring.security.oauth2.resourceserver.jwt.jwk-set-uri', '${server.name}/cas/oidc/jwks', '${server.name}/cas/oidc/jwks', '', 'security', 'service-manager', '40', '120', '10', '1', '1', '1', '0', '0', '0', 'text', 'text', '', '1', '', 'JWK URI', CURRENT_TIMESTAMP, '-1' )
...
ALTER TABLE dict_enrollments ALTER COLUMN domain_name TYPE varchar (255), ALTER COLUMN domain_name SET NOT NULL;

UPDATE SYS_CONFIG SET UI = TRUE WHERE APPLICATION  = 'application' AND CONFIG_KEY = 'help.url.base';
...
CREATE VIEW AGENT_INFO AS (SELECT
	...
	SELECT
		EXTRACT(EPOCH
	FROM
		CURRENT_TIMESTAMP) * 1000) THEN 'ACTIVE' ELSE 'INACTIVE' END STATUS,
	CASE WHEN (
	SELECT
		COALESCE(MAX(AS_OF), 0)
	FROM
		DEPLOYMENT_RECORDS) <= AGENT.DEPLOYMENT_BUNDLE_TS THEN 'UP_TO_DATE' ELSE 'OUT_OF_DATE' END POLICY_STATUS
FROM
	AGENT);
